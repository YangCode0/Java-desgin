 ...
 * 虽然在 if 语句块内有加锁操作，但是两个线程都会执行 instance = new Singleton();
 * 这条语句，只是先后的问题，那么就会进行两次实例化。
 * 因此必须使用双重校验锁，也就是需要使用两个 if 语句：第一个 if 语句用来避免 instance 已经被实例化之后的加锁操作，而第二个 if 语句进行了加锁，所以只能有一个线程进入，
 * 就不会出现 instance == null 时两个线程同时进行实例化操作。
 *
